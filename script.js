...
function toggleTheme(){
  ...
  if(current === 'light'){
    ...
    document.getElementsByTagName('ion-icon')[0].setAttribute('name', 'sunnt-outline');
...
function selectRegions(){
  ...
  const selectedRegion = list.options[list.selectedIndex].text.tolowerCase();
  const regions = document.getElementsByTagName('li');

  for(var i =1; i < regions.length; i += 3){
    if(regions[i].innerText.tolowerCase().indexOf(selectedRegion) != -1){
...
//JSON =  format for storing and trabsporting data and used to sent data from server to web pages
// async()= return promise based behavior
// await= make promise to return the fun. which bahave like they're sync and suspend the execution until return promise is accepted or rejected.
// fetch() = request to the server and load data on webpages
//JSON.stringfy() =  converts javascript object or value to json string

fetchCountries = async() =>{
  const response = await fetch('https://restcountries.com/v2/all');
  const myJson = await response.json(); //extract json from http response
  console.log(myJson);
...
//addEventListener = matches attaches an event handler to an element
...
function setData(myJson, children, section, recursion){
	...
	for(var i = 0; i < myJson.length; i++){
		if(myJson[i].name.toLowerCase() === (recursion ? children.toLowerCase() : children[1].innerText.toLowerCase())){
            ...
            const ul = document.getElementById('neighbor-countries')
            ul.innerHTML = '';
